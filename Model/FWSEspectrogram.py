...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, in_channel=80,inplane=128, zero_init_residual=False,
        ...
        self.conv1 = nn.Conv2d(in_channel, self.inplanes, kernel_size=3, stride=(2,1), padding=1)
        self.bn = norm_layer(64)
        self.relu = nn.ReLU()
        self.layer1 = self._make_layer(block, 64, layers[0],stride=(2,2),frequency=65)
        self.layer2 = self._make_layer(block, 128, layers[1], stride=(2,2),frequency=33,
                                       dilate=1)
        self.layer3 = self._make_layer(block, 256, layers[2], stride=(2,2),frequency=17,
                                       dilate=1)
        self.layer4 = self._make_layer(block, 512, layers[3], stride=(2,2),frequency=9,
                                       dilate=1)
...
class ResNet34FWSESpectrogram(nn.Module):
    def __init__(self,window_length=400,hopping_length=160, mel_number= 80, fft_size= 256, window_function=torch.hamming_window):
            ...
            self.fc = nn.Sequential(nn.Linear(18*512,2048),
                                    nn.ReLU(),
                                    nn.Linear(2048,512))
            self.attention = nn.Sequential(
                nn.Conv1d(
                    9*512, 512, kernel_size=1),
                nn.ReLU(),
                nn.BatchNorm1d(512),
                nn.Conv1d(
                    512, 9*512, kernel_size=1),
                nn.Softmax(dim=2), )
